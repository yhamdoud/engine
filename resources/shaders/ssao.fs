...
void main()
{
    ...
    for (uint i = 0; i < u_kernel_size; i++)
    {
        ...
        // Project sample to screen (clip) space to get depth buffer
        // coordinates.
        ...
        float depth_at_sample =
            linearize_depth(texture(u_g_depth, sample_coord.xy).r, u_proj);

        // Deals with samples that are not located near the original fragment,
        // e.g., samples near the edge.
        float range_check =
            smoothstep(0., 1., u_radius / abs(frag_depth - depth_at_sample));

        // Check if the sample is occluded by comparing it's depth with the
        // depth value sampled at it's position. Update the occlusion count
        // accordingly.
        occlusion +=
            (depth_at_sample >= sample_pos.z + u_bias ? 1. : 0.) * range_check;
    }
